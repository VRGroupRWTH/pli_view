  ...
  connect(button_calculate            , &QAbstractButton::clicked   , [&]
  {
    ...
    future_ = std::async(std::launch::async, [&]
    {
      try
      {
        ...
        std::vector<float3> vectors_linear(vectors.data(), vectors.data() + vectors.num_elements());
        polar_plots = calculate(vectors_linear, vectors_dimensions, superpixel_size, angular_partitions, symmetric);
      }
