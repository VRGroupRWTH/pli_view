...
selector_plugin::selector_plugin(QWidget* parent) : plugin(parent)
{
  ...
  {
    auto value = std::min(line_edit_utility::get_text<std::size_t>(line_edit_size_z), std::size_t(slider_z->maximum() - slider_z->minimum()));
  ...
  {
    std::array<int, 2> offset { offset_perc[0] * slider_x->maximum(), offset_perc[1] * slider_y->maximum() };
    std::array<int, 2> size   { size_perc  [0] * slider_x->maximum(), size_perc  [1] * slider_y->maximum() };
