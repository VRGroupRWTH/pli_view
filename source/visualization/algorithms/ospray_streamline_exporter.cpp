  ...
  const std::string& filepath)
{
  ...
  //auto light_vector = std::vector<osp::Light>{ambient_light.handle(), distant_light.handle()};
  
  auto handle = ambient_light.handle();
  ospray::cpp::Data lights(1, OSP_LIGHT, &handle);
  lights  .commit();
