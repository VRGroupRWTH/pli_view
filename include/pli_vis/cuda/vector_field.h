        ...
        color_type*  colors      )
{
  ...
  auto vector_start    = pli::to_cartesian_coords(float3{ scale * minimum_spacing / 2.0F, longitude, latitude});
  auto vector_end      = pli::to_cartesian_coords(float3{-scale * minimum_spacing / 2.0F, longitude, latitude});
  auto unscaled        = pli::to_cartesian_coords(float3{1.0F, longitude, latitude});
        ...
        color_type*  colors      )
{
  ...
  auto        vector_spherical = pli::to_spherical_coords(vector);
  ...
  auto minimum_spacing = float(min(spacing.x, min(spacing.y, spacing.z)));
  auto vector_start    = pli::to_cartesian_coords(float3{ scale * minimum_spacing / 2.0F, vector_spherical.y, vector_spherical.z});
  auto vector_end      = pli::to_cartesian_coords(float3{-scale * minimum_spacing / 2.0F, vector_spherical.y, vector_spherical.z});
  auto unscaled        = pli::to_cartesian_coords(float3{1.0F, vector_spherical.y, vector_spherical.z});
  // auto color        = make_float4(abs(unscaled.x), abs(unscaled.y), abs(unscaled.z), 1.0); // Default
  auto color           = make_float4(abs(unscaled.x), abs(unscaled.z), abs(unscaled.y), 1.0); // DMRI
