...
public:
  ...
  {
    glFramebufferTexture1D(target, attachment, GL_TEXTURE_1D, texture.id(), level);
  }
  void        texture_2d           (GLenum attachment, const texture_2d& texture, GLuint level = 0)
  {
    glFramebufferTexture2D(target, attachment, GL_TEXTURE_2D, texture.id(), level);
  }
  void        texture_3d           (GLenum attachment, const texture_3d& texture, GLuint level = 0, GLuint layer = 0)
  {
    glFramebufferTexture3D(target, attachment, GL_TEXTURE_3D, texture.id(), level, layer);
  }
